//! Transitional publisher wire format: the JSON shape returned by GET /state.
//!
//! Publishers are stateless. They report current state. History is the
//! aggregator's problem.
//!
//! This module preserves the composite `nq.witness_packet.v1` envelope while
//! the runtime is decomposed. Its family-specific fields and report DTOs are
//! historical compatibility debt. They are not a generic check-pack output
//! contract and do not establish independent schema ownership for the packs
//! that currently consume them.
...
/// `nq.witness.v1`, which is owned by the witness artifact package and is a
/// different surface.
...
/// Closed compatibility projection of the collector families embedded in the
/// v1 snapshot. New pack composition must not require a new field here.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// of a typed refusal. Closed vocabulary: only fields that can actually
...
pub struct MetricSample {
    ...
    /// (e.g. "blackbox_primary_endpoint"). Additive; older payloads
...
// Mirrors the public nq.witness.v0 / nq.witness.zfs.v0 profile.
// ---------------------------------------------------------------------------
...
// Mirrors the public nq.witness.v0 / nq.witness.smart.v0 profile.
//
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(tag = "kind")]
// Boxing would change the established public Rust API for this versioned wire.
