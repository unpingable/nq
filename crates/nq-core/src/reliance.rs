...
//!
//! - reliance consumes an already-sealed [`Receipt`] and never re-evaluates
//!   evidence — there is no code path here that inspects raw observations;
...
impl ProfileCatalog {
    ...
    pub fn get(&self, id: &str) -> Option<&ConsumerProfile> {
        self.profiles
            .iter()
            .find(|p| p.consumer_profile_id == id)
    }
...
fn mandatory_does_not_establish() -> Vec<String> {
    vec![
        ...
        "this decision names no action and licenses no retry, clearing, or escalation"
            .to_string(),
...
fn evaluate(
    ...
    match receipt.content_hash.as_deref() {
    ...
    if receipt.status_reasons.contains(&StatusReason::StaleObservation) {
    ...
    if receipt.status_reasons.contains(&StatusReason::MissingRequiredClaim) {
...
mod tests {
    ...
    fn source_receipt_is_untouched_and_consumer_neutral_across_consumers() {
        ...
        let _ = decide_with(&request("operator-review", "review", &rec), &rec, &EvidenceContext::default());
        let _ = decide_with(
            &request("nightshift-readonly", "wait", &rec),
            &rec,
    ...
    fn changing_consumer_changes_reliance_identity() {
        let rec = sealed_receipt(Status::Verified, vec![]);
        let a = decide_with(&request("operator-review", "review", &rec), &rec, &EvidenceContext::default());
        let b = decide_with(
            &request("nightshift-readonly", "wait", &rec),
            &rec,
            &EvidenceContext::default(),
    ...
    fn changing_purpose_changes_reliance_identity() {
        ...
        let b = decide_with(&request("nightshift-readonly", "wait", &rec), &rec, &EvidenceContext::default());
        assert_ne!(a.decision_id, b.decision_id);
    }
    ...
    fn operational_health_reliance_does_not_establish_total_nq_correctness() {
        ...
        assert!(out.establishes.iter().all(|e| !e.contains("nq_trustworthy")));
    ...
    fn no_action_is_emitted_by_any_outcome() {
        ...
        for (profile, purpose) in [
            ...
            let out = decide_with(&request(profile, purpose, &rec), &rec, &EvidenceContext::default());
            let text = serde_json::to_string(&out).unwrap();
    ...
    fn shipped_example_catalog_loads_and_matches_the_documented_profiles() {
        ...
        let ns = catalog.get("nightshift-readonly").expect("nightshift profile");
    ...
    fn catalog_refuses_unknown_schema_and_duplicate_profiles() {
        let bad_schema = br#"{"schema":"nq.reliance.profiles.v99","policy_version":"v1","profiles":[]}"#;
    ...
    fn caller_binding_is_never_described_as_authenticated() {
        ...
        assert!(out.caller_binding_disclosure.contains("not an authenticated"));
        let text = serde_json::to_string(&out).unwrap();
