...
async fn smoke_disk_state_passes_on_seeded_forcing_case() {
    use nq_monitor::http::routes::router;
    use nq_monitor::smoke::validate_disk_state_envelope;
...
async fn dns_state_http_nxdomain_emits_admissible_with_scope_with_bounded_wording() {
    ...
    // hand-rolled axum app — `nq_monitor::http::routes::router` is what
    // `nq serve` ships.
    use nq_monitor::http::routes::router;
...
fn sqlite_wal_probe_pipeline_end_to_end_smoke() {
    ...
    let publisher_cfg = PublisherConfig {
        ...
        // Pipeline smoke disables /proc/locks enrichment so the
        // observed-row assertion below ("proc_access == not_attempted")
        // stays deterministic across CI environments. The
