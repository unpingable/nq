//! `nq-monitor receipt {render,check}` — operate on existing `nq.receipt.v1`
...
//!   produced by `nq-monitor verify` and render it as markdown for a PR comment.
...
fn check(cmd: ReceiptCheckCmd) -> anyhow::Result<()> {
    ...
    let code = match check_impl(cmd) {
        Ok(c) => c,
        Err(e) => {
            eprintln!("nq-monitor receipt check: {e:?}");
            64
        }
    };
...
// =================================================================
// `nq-monitor receipt replay` — Slice 1e
// =================================================================
...
fn replay(cmd: ReceiptReplayCmd) -> anyhow::Result<()> {
    let code = match replay_impl(cmd) {
        Ok(c) => c,
        Err(e) => {
            eprintln!("nq-monitor receipt replay: {e:?}");
            64
        }
    };
    std::process::exit(code);
