//! `nq-monitor smoke` — operator-facing contract smokes against the running
...
use crate::cli::{
    SmokeAction, SmokeCmd, SmokePreflightDiskStateCmd, SmokePreflightIngestStateCmd,
};
use crate::smoke::{validate_disk_state_envelope, validate_ingest_state_envelope};
...
pub fn run(cmd: SmokeCmd) -> anyhow::Result<()> {
    match cmd.action {
        ...
        SmokeAction::PreflightIngestState(c) => run_preflight_ingest_state(c),
    }
}
...
fn run_preflight_ingest_state(cmd: SmokePreflightIngestStateCmd) -> anyhow::Result<()> {
    let base = cmd.url.trim_end_matches('/');
    let url = format!("{base}/api/preflight/ingest-state");
    ...
    // The ingest_state route returns the PreflightResult JSON directly
    // — there is no nested `*_preflight` envelope key like disk_state's
    // /api/host/<host> response. The witness is the monitor itself, so
    // no per-host wrapping is meaningful.
    let envelope: Value = resp
        .json()
        .with_context(|| format!("GET {url} returned non-JSON body"))?;

    let report = validate_ingest_state_envelope(&envelope)?;
