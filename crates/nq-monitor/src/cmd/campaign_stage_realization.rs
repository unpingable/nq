//! Read-only transport for exact reservation-realization qualification.

use crate::cli::{
    CampaignStageRealizationAction, CampaignStageRealizationCmd,
    CampaignStageRealizationEvaluateCmd, CampaignStageRealizationReplayCmd,
};
...
use nq_core::{
    ...
    CampaignStageRealizationReceiptV2, QualificationEvaluatorIdentityV1,
};
...
const EVALUATOR_ID: &str = "nq.campaign-stage-realization-evaluator/v2";

pub fn run(cmd: CampaignStageRealizationCmd) -> anyhow::Result<()> {
    match cmd.action {
        CampaignStageRealizationAction::Evaluate(cmd) => evaluate(cmd),
        CampaignStageRealizationAction::Replay(cmd) => replay(cmd),
    }
}

fn evaluate(cmd: CampaignStageRealizationEvaluateCmd) -> anyhow::Result<()> {
    let profile: CampaignStageRealizationProfileV2 = read_json(&cmd.profile)?;
    let evidence: CampaignStageRealizationEvidenceV2 = read_json(&cmd.evidence)?;
    let receipt = evaluate_campaign_stage_realization(
        &profile,
        &evidence,
...
fn replay(cmd: CampaignStageRealizationReplayCmd) -> anyhow::Result<()> {
    let profile: CampaignStageRealizationProfileV2 = read_json(&cmd.profile)?;
    let evidence: CampaignStageRealizationEvidenceV2 = read_json(&cmd.evidence)?;
    let receipt: CampaignStageRealizationReceiptV2 = read_json(&cmd.receipt)?;
    ...
    let result = replay_campaign_stage_realization(&profile, &evidence, &receipt);
    write_json(&cmd.output, &result)?;
    if !result.matches {
        bail!("campaign-stage realization replay did not reproduce the receipt");
    }
    Ok(())
}
