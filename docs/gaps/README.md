...
- [`TESTIMONY_DEPENDENCY_GAP`](TESTIMONY_DEPENDENCY_GAP.md) — `partial`. Findings inherit admissibility through the testimony chain that produced them. When an interior node (host, witness, transport, collector) loses observability, descendants transition to suppressed-with-last-state, not auto-cleared. **V1.0 shipped 2026-04-28**: `*_witness_silent` finding kinds wired into the existing masking layer with kind-prefix scoping; `witness_unobservable` joins `host_unreachable` and `source_unreachable` as a suppression reason. **V1.1 pending**: producer_ref column, paired `node_unobservable` kind, admissibility view, multi-level ancestry. Role-derived severity reserved for REGISTRY_PROJECTION.
...
- [`COVERAGE_HONESTY_GAP`](COVERAGE_HONESTY_GAP.md) — `partial`. Liveness, coverage, and truthfulness are three axes; green on one does not imply the others. **V1.0 shipped 2026-04-28**: migration 038 + 12 typed envelope columns, `coverage_degraded` and `health_claim_misleading` finding kinds with `finding_meta` entries, `RecoveryState`/`RecoveryComparator` types, publish-path persistence, five round-trip tests. Clearance path 2 (ancestor-suppression) live via TESTIMONY_DEPENDENCY V1.0; path 1 (explicit recovery testimony) is producer-driven. **V1.1 pending**: JSON export wiring, composition validation for `health_claim_misleading`.
