...
- [`TESTIMONY_DEPENDENCY_GAP`](TESTIMONY_DEPENDENCY_GAP.md) — `partial`. Findings inherit admissibility through the testimony chain that produced them. When an interior node (host, witness, transport, collector) loses observability, descendants transition to suppressed-with-last-state, not auto-cleared. **V1.0 + V1.1 (admissibility view + JSON wire surface) shipped 2026-04-28**: `*_witness_silent` finding kinds wired into masking with kind-prefix scoping; `witness_unobservable` joins `host_unreachable` and `source_unreachable` as a suppression reason; migration 039 creates `v_admissibility`; `FindingSnapshot.admissibility` block carries `state` / `reason` / `ancestor_finding_key` end-to-end. **Pending**: producer_ref column, paired `node_unobservable` kind, multi-level ancestry. Role-derived severity reserved for REGISTRY_PROJECTION.
...
- [`OPERATIONAL_INTENT_DECLARATION_GAP`](OPERATIONAL_INTENT_DECLARATION_GAP.md) — `proposed`. Substrate primitive for declared expectation mutation. Two modes: `quiesced` (subject visible, work intake stops) and `withdrawn` (subject removed from active expected surface). Orthogonal axis to TESTIMONY_DEPENDENCY: declaration changes *expectation*, ancestry-loss changes *standing*. NQ records intent; NQ does not act on the world. Maintenance declarations are one profile. Persistent declarations without review become haunted furniture — hygiene detectors fire on stale, conflicting, or scope-ambiguous declarations. V1: declaration storage, suppression metadata on findings, three hygiene detectors, no runtime control.
