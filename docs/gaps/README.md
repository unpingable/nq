...
### Consumer contract
- [`FINDING_EXPORT_GAP`](FINDING_EXPORT_GAP.md) — `built, shipped (V1 surface)`. Canonical `FindingSnapshot` DTO + `nq findings export` CLI. Identity + lifecycle + diagnosis + regime + observations + generation-context + admissibility + coverage + node_unobservable + basis as one versioned object. **V1 surface shipped 2026-04-16 → 2026-05-01**: DTO, read helper, CLI subcommand, schema preflight, plus admissibility / coverage / node_unobservable / basis / extended regime payloads via TESTIMONY V1.1/V1.2, COVERAGE V1.1, OPERATIONAL_INTENT V1, EVIDENCE_RETIREMENT V1. Findings are evidence, not commands. Cross-repo Night Shift end-to-end validation (acceptance criterion #11, in `~/git/scheduler`) and acceptance-criteria coverage-map audit remain pending.
