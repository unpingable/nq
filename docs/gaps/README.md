...
- [`TESTIMONY_DEPENDENCY_GAP`](TESTIMONY_DEPENDENCY_GAP.md) — `proposed`. Findings inherit admissibility through the testimony chain that produced them. When an interior node (host, witness, transport, collector) loses observability, descendants transition to `suppressed_by_ancestor` — last-known state preserved, admissibility revoked, not auto-cleared. V1: observability-loss collapse, one promoter (`smart_witness_silent`), one consumer (COVERAGE_HONESTY clearance contract). Role-derived severity reserved, defers to REGISTRY_PROJECTION.
