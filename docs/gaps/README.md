...
- [`TESTIMONY_DEPENDENCY_GAP`](TESTIMONY_DEPENDENCY_GAP.md) — `partial`. Findings inherit admissibility through the testimony chain that produced them. When an interior node (host, witness, transport, collector) loses observability, descendants transition to suppressed-with-last-state, not auto-cleared. **V1.0 + V1.1 admissibility view shipped 2026-04-28**: `*_witness_silent` finding kinds wired into masking with kind-prefix scoping; `witness_unobservable` joins `host_unreachable` and `source_unreachable` as a suppression reason; migration 039 creates `v_admissibility` exposing the canonical `observable` / `suppressed_by_ancestor` axis with `ancestor_reason`. **Pending**: producer_ref column, paired `node_unobservable` kind, multi-level ancestry. Role-derived severity reserved for REGISTRY_PROJECTION.
