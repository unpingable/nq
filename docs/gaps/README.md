...
- [`DURABLE_ARTIFACT_SUBSTRATE_GAP`](DURABLE_ARTIFACT_SUBSTRATE_GAP.md) — `proposed`. Admit extraction-derived durable artifact corpora as a witnessable substrate class, distinct from live host-bound axes (CPU/memory/disk/network). Pins inbound-testimony pipeline boundary (mirror of FINDING_EXPORT, inverted), two-clock provenance (producer extraction time vs NQ ingest time), and composition-first finding shape (durable-artifact concerns express as profiles of COVERAGE_HONESTY / SILENCE_UNIFICATION / OPERATIONAL_INTENT_DECLARATION / TESTIMONY_DEPENDENCY, not parallel kinds). Admission contingent on consumer-alignment test (NS/Governor/AG read these through existing admissibility pathways) and a real producer materializing. V1 = synthetic-producer fixture + one detector composition; PROVENANCE_GRAPH_PROFILE deferred. Triggered by storage and provenance pressure, but not a storage gap.
