...
### 5. `stability` — temporal-coherence axis (roadmapped, not yet shipped)

**Values:** `stable` / `flapping`
**Owner:** to be filed; the `service_flap` detector exists but the per-finding `stability` axis is not yet a substrate column.
**Source:** README, marked *(roadmapped)*. Per-detector flap awareness is partial today.
**Refusal it encodes (when shipped):** "stable" and "flapping" are different kinds of trouble. A flapping finding is not "intermittently fine"; it is unstable. The axis must not collapse into a count-of-transitions metric.

### 6. Notification recurrence — projection-time marker, **not** an axis
...
### 7. Future axis: responder-state — reserved doctrinal slot, not a planned build
...
**Diagnosis:** two severity vocabularies, no shared parent. Persistence-escalation is producing the `critical` label by counting generations. Posture is producing `investigate business hours` from a separate field that knows this class of finding does not warrant 2am paging.

**Resolution (specified here, not built):** the model must distinguish:

- **severity-of-condition** — substrate-derived; how bad the substrate state currently is. Persistence-escalation is one input. `freelist_bloat` at 21d may legitimately read as `critical` on this axis.
- **urgency-of-response** — operator-derived; whether this warrants immediate response *given posture, runbook, time-of-day, and on-call state*. `investigate business hours` lives here.

The bug is not "the header is wrong." The bug is that the dashboard's header projects severity-of-condition while the posture line projects urgency-of-response and the surface treats them as the same value. Per the keystone refusal: never let one register render as the other.

This document does not propose the urgency-axis implementation. It names the laundering as a class and locks the resolution shape (two separate scalars; surfaces that render one MUST NOT label it with the other's vocabulary).
...
|---|---|---|---|---|
| Dashboard "Open Findings" list | condition + visibility | — | severity-of-condition | render `basis_state ∈ {stale, retired, invalidated, unknown}` in the same visual class as `live` |
| Dashboard header | (TBD per persistence laundering) | — | severity-of-condition AND urgency-of-response, distinctly | collapse the two severities into one number |
| Slack / webhook notification | condition + basis_state | `(new)` / `(recurring)` | severity-of-condition | drop the recurrence marker; render suppressed findings as live |
| `nq finding list` CLI | filterable by all axes | — | severity-of-condition | default to condition=open and silently hide other axes |
...
- No proposal to ship the urgency-of-response axis. The persistence-laundering example names the bug; implementing the fix is downstream slice work.

## Open questions

1. **Should `stability` get a dedicated `warning_state` column** when shipped, or compose from `service_flap` + transition-history queries? Either is admissible; the axis is the thing this document protects, not the storage shape.
