...
- `service_state` / `service_recovered` — named-but-not-built per [PATH_TO_1_0.md](../working/decisions/PATH_TO_1_0.md). Witness shape is undecided; specifically there is no recovery witness in current NQ, and a liveness-only witness is not permitted to testify to recovery. See [CLAIM_PREFLIGHT_EXISTING_WITNESSES.md](../working/decisions/CLAIM_PREFLIGHT_EXISTING_WITNESSES.md) §"Future candidate claim kinds" for the underlying rule.
...
- [CLAIM_PREFLIGHT.md](../working/decisions/CLAIM_PREFLIGHT.md) — doctrine.
