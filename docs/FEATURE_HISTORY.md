...
## REGIME_FEATURES partial-V1

**Status:** partial — shipped. Five of the six feature classes from the V1 spec are live and consumed in production: trajectory, persistence, recovery, co-occurrence, resolution. Plus the badge surface that summarizes them for operators. **§5 observability remains pending** as a first-class feature class — only an indirect `RegimeHint::ObservabilityFailure` variant emitted via co-occurrence pairs exists today, not the typed `signal_silence_generations` / `expected_metric_missing` / `evidence_basis` shape the spec calls for. Shipped across 2026-04-14 → 2026-04-20. Ratified under the gap-status discipline 2026-05-07 (this entry).
...
- Compute pass: `crates/nq-db/src/regime.rs::compute_features` (line 1219). Single transactional sweep per generation: `compute_host_trajectories` → `compute_finding_persistence` → `compute_finding_recovery` → `compute_finding_co_occurrence` → `compute_host_resolution`. Each writer uses `upsert_feature` so re-running the pass on the same generation is idempotent.
...
- Resolution: `compute_host_resolution` (line 879), `plateau_depth` (line 836), `classify_recovery_phase` (line 870). Reads host metric history; emits per-(host, metric) Acute / Improving / Settling based on direction + plateau against a prior peak. `SteadyState` is deliberately never emitted in V1 — the strict claim requires `reuse_behavior` and `residual_anomaly_class` which the V1 subset does not compute (test `resolution_never_emits_steady_state_in_v1`).
- Badge surface: `RegimeBadge` enum (line 1342), `derive_regime_badge` (line 1377), `badge_explanation` (line 1408). Priority order: pathological recovery → Worsening; any host metric Acute → Worsening; any Improving/Settling → Resolving; entrenched + Normal/Slow recovery → Stable; otherwise None. Pure function over already-read payloads (testable without a `ReadDb`).
- Read helpers: `latest_finding_persistence` (line 1242), `latest_finding_recovery` (line 1263), `latest_host_co_occurrence` (line 1285), `latest_host_resolution` (line 1309). Used by both renderers and notifier.
- Tests: 90 `#[test]` functions in `crates/nq-db/src/regime.rs::tests` covering trajectory (6), persistence (8), recovery (24 — including `recovery_pathological_not_masked_by_self_pollution` for the prior-cycles-only median invariant), co-occurrence (10), resolution (8), badge (7), plus shared utility tests. Every spec acceptance criterion #1–#3 (named computation pass; append-only output path; consumers depend on derived facts) is covered; #4 (trajectory/persistence/recovery/co-occurrence/resolution as first-class types) is satisfied for five of the five named-here classes; #6 (basis/window metadata) is enforced at the `upsert_feature` write site; #7 (generation as primary clock) is structural; #8 (no general-purpose TSDB) holds — `regime_features` stores derived facts only, the underlying history is owned by `hosts_history` / `metrics_history` / `finding_observations`.
...
- **§5 observability not first-class.** No `signal_silence_generations`, no `expected_metric_missing`, no `evidence_basis`. The spec's intent — "prevent NQ from treating silence as health; make uncertainty structurally visible" — is partially served via co-occurrence pairs that emit `RegimeHint::ObservabilityFailure`, but the typed feature class for direct silence accounting is not built. Filed as a separate slice (this session's task).
...
- **No retention horizon yet.** Spec §"Design Stance" mandates per-feature-class retention coupled to the existing prune pass. `regime_features` rows still rely on `ON DELETE CASCADE` with `generations` and the existing retention loop. A dedicated regime-features prune horizon is its own follow-up; cardinality has not yet forced the issue. The "must preserve most-recent-sufficient-history row per (subject_kind, subject_id, feature_type)" invariant is not yet enforced — the existing recompute upsert pattern keeps the most recent row by virtue of cascade-newest-row semantics, but a prune pass that drops mid-window history could lose interpretive value before formal retention lands.
...
**Unblocks:**
- Notification routing — STABILITY_AXIS V1 is one prerequisite; this gap is the second. Routing remains stub-deferred behind both, but with §5 observability as the live remaining piece, the dependency surface is one slice from being clean.
...
**Field notes:**
- The "slice stop" commit (`1736142`) is a load-bearing discipline marker. After resolution shipped, the temptation was to keep building (richer renderer surface, observability slice, retention horizon) before any real consumer used what was already there. The slice-stop instead committed exactly one badge + one notifier sentence and left the rest for forcing-case-driven follow-ups. Three weeks later (this entry), the same pattern is showing up as the right call: the badge surface is being consumed, the spec gap (§5 observability) is being filed as a deliberate next slice rather than discovered as an unbuilt corner.
- `derive_regime_badge` and `badge_explanation` are pure functions over already-read payloads. The decision to take payloads as arguments (not a `ReadDb`) made the badge surface unit-testable cheaply (7 tests at `regime.rs:2841-2895`) and moves the I/O concern to the caller. Same pattern as `apply_action_bias_elevation` in DOMINANCE V1.x (`40bcac7`); consistent shape across the project.
- Co-occurrence's `CO_OCCURRENCE_SIGNATURES` const table follows the same data-driven pattern as `MASKING_RULES` (GENERALIZED_MASKING V1) — declarative table, single source of truth, lookup fn (`lookup_regime_hint`) is pure. Adding a new regime hint is one entry, not a code change in the loop.
- The recovery layer's "prior-cycles-only median" insight (`cf16706`, the chatty-fix split_last refactor) was a real load-bearing review catch. A naive median-over-all-cycles would have made pathological cycles dampen themselves into Slow or Normal by polluting the very baseline they're being compared against. The test `recovery_pathological_not_masked_by_self_pollution` is the regression guard. Worth knowing if anyone touches the median computation.
