...
- [`project_nq_witness_daemon_trajectory`](file:///home/jbeck/.claude/projects/-home-jbeck-git-notquery/memory/project_nq_witness_daemon_trajectory.md) — the four-verb layering (observe / evaluate / correlate / authorize). NQ's `observe` and `evaluate` lanes are upstream of NS's closure assessment lane; the recursion rule applies.
