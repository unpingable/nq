...
**Status:** `built, shipped (V1)` 2026-05-12. See [`FEATURE_HISTORY.md` § DURABLE_ARTIFACT_SUBSTRATE V1](../decisions/FEATURE_HISTORY.md#durable_artifact_substrate-v1-synthetic-producer-slice). **Substrate-class admission ratified 2026-05-12 by NS consumer-alignment dry run (outcome (a) per §V1 step 6 — NS V1.x parses the synthetic-producer output without `NqInadmissible`-shaped refusal under existing admissibility branching).** Real-producer ingestion + PROVENANCE_GRAPH_PROFILE + `dependency_cone_changed` + corpus-shaped subject-identity vocabulary remain V2+/profile concerns. Three "NS tolerates but does not consume" seams surfaced by the dry run (two-clock latency, silence-envelope invisibility, origin-envelope invisibility) — informational; not blocking V1 closure; named in FEATURE_HISTORY as the seams that ratification produced.
...
- `docs/architecture/SCOPE_AND_WITNESS_MODEL.md` §Core scope — substrate axes are currently host-bound; this gap admits a class that isn't.
- `docs/working/gaps/FINDING_EXPORT_GAP.md` — outbound consumer contract; this gap is the inbound mirror, contract-shaped not symmetric.
- `docs/working/gaps/COVERAGE_HONESTY_GAP.md` §Design Stance — three-axis model that durable-artifact coverage profiles compose onto.
- `docs/working/gaps/TESTIMONY_DEPENDENCY_GAP.md` §Design Stance — ancestor-suppression discipline that ingested-finding producer-loss reuses.
- `docs/working/gaps/OPERATIONAL_INTENT_DECLARATION_GAP.md` — declared-expectation primitive that durable-artifact heuristic-review concerns compose onto.
- `docs/working/gaps/SILENCE_UNIFICATION_GAP.md` — silence contract that V1's `extraction_stale` detector instantiates.
- `docs/working/gaps/STORAGE_BACKEND_GAP.md` — orthogonal: scaling NQ's own store. This gap is about NQ ingesting from a substrate it does not own.
