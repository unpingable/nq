...
**Status:** `partially resolved` 2026-06-02 — §2 (co-residence trigger) is shipped-state in [`FEATURE_HISTORY.md` § WITNESS_EVALUATOR_BOUNDARY Track 4](../decisions/FEATURE_HISTORY.md#witness_evaluator_boundary-track-4). The architectural commitment (structural W/E separation) is now backed by Rust's link boundary, not operator discipline. §1, §3, §4, §5, §6 discipline lines below remain in force; in-process co-residence inside `nq-monitor serve`'s pulse loop for the component-testimony heartbeat is still permitted as bounded defense-in-depth, not the architectural commitment.
