...
- docs/working/gaps/REGIME_FEATURES_GAP.md — supplies temporal class (transient / persistent / entrenched), dominance, and trajectory that the render consumes
- docs/working/gaps/FINDING_DIAGNOSIS_GAP.md — the typed finding nucleus the render speaks from
- docs/working/gaps/STABILITY_AXIS_GAP.md — stability primitives feeding persistence class
- docs/working/gaps/NOTIFICATION_ROUTING_GAP.md (stub) — routing operates upstream; rendering is downstream
- docs/working/gaps/NOTIFICATION_INHIBITION_GAP.md (stub) — inhibition operates upstream; rendering does not dedupe
- docs/working/gaps/ACTION_OVERLAY_GAP.md (stub) — where machine-action rendering will eventually live, with authority-state
- docs/working/gaps/HUMAN_PROCEDURE_OVERLAY_GAP.md (stub) — where procedure linkage will eventually live, with ownership/lifecycle
