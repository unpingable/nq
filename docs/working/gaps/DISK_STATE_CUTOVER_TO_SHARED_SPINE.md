...
- **Dedicated route shape:** does `GET /api/preflight/disk-state/{host}` continue to exist post-cut-over, or does it collapse into `nq verify disk_state --host ...` over the shared receipt surface? The two answers have different operator-tooling implications.
