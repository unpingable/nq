...
**Status:** **all nine V1 detectors live** as of 2026-07-01. Phase A/B/C shipped the original five 2026-04-20 (`zfs_pool_degraded`, `zfs_vdev_faulted`, `zfs_error_count_increased`, `zfs_scrub_overdue`, `zfs_witness_silent`). 2026-07-01 added the four previously forcing-case-gated detectors — two current-state (`zfs_pool_suspended`, `zfs_pool_capacity_pressure`) and two cross-generation transition detectors (`zfs_pool_health_changed`, `zfs_spare_activated`) backed by new `zfs_pools_history` / `zfs_spares_history` substrate (migration 063, mirroring `zfs_vdev_errors_history`). All built against **lab witness fixtures** per the recognition-coverage doctrine — compatibility evidence, not live testimony; live authority still requires real-estate deployment. The detector-completeness half of V1 is done; the live worked-example on `lil-nas-x` remains a deployment/observation exercise, not a code gap. See [../decisions/FEATURE_HISTORY.md#zfs_collector-phases-abc](../decisions/FEATURE_HISTORY.md#zfs_collector-phases-abc) for evidence (migrations 031 + 032 + witness consumer + coverage-gated detectors + REGIME co-occurrence wiring + live lil-nas-x DEGRADED-stable classification). Ratified 2026-05-07.
...
- `docs/working/gaps/OBSERVER_DISTORTION_GAP.md` — Δq participation discipline. All three witness privilege models (`sudo_helper`, `root_exporter_localhost`, `unprivileged`) are valid boundary patterns; NQ stays unprivileged in every case.
- `docs/working/gaps/PORTABILITY_GAP.md` — capability manifest. ZFS collector declares Linux-only in v1.
- `docs/working/gaps/STABILITY_AXIS_GAP.md` — chronic-degraded vs degrading distinction lives here.
- `docs/working/gaps/REGIME_FEATURES_GAP.md` — persistence + recovery regime features that make a persistent DEGRADED finding legible without becoming panic theater.
- `docs/working/gaps/FINDING_EXPORT_GAP.md` — ZFS findings flow through the same consumer contract.
