...
**Status:** partial — Phase A (witness ingestion spine) + Phase B (first detector cashout) + Phase C (worsening + regime integration) shipped 2026-04-20. **Seven of nine V1 detectors live** as of 2026-07-01: the original five (`zfs_pool_degraded`, `zfs_vdev_faulted`, `zfs_error_count_increased`, `zfs_scrub_overdue`, `zfs_witness_silent`) plus the two current-state pool detectors `zfs_pool_suspended` + `zfs_pool_capacity_pressure` (built against lab witness fixtures per the recognition-coverage doctrine — compatibility evidence, not live testimony). The two remaining (`zfs_pool_health_changed`, `zfs_spare_activated`) are cross-generation *transition* detectors that need pool-state / spare history substrate (only `zfs_vdev_errors_history` exists today) — a separate slice. See [../decisions/FEATURE_HISTORY.md#zfs_collector-phases-abc](../decisions/FEATURE_HISTORY.md#zfs_collector-phases-abc) for evidence (migrations 031 + 032 + witness consumer + coverage-gated detectors + REGIME co-occurrence wiring + live lil-nas-x DEGRADED-stable classification). Ratified 2026-05-07.
