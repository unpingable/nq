...
**Status:** partial — Phase A (witness ingestion spine) + Phase B (first detector cashout) + Phase C (worsening + regime integration) shipped 2026-04-20. Five of nine V1 detectors live (`zfs_pool_degraded`, `zfs_vdev_faulted`, `zfs_error_count_increased`, `zfs_scrub_overdue`, `zfs_witness_silent`); four forcing-case-gated (`zfs_pool_suspended`, `zfs_pool_health_changed`, `zfs_pool_capacity_pressure`, `zfs_spare_activated`). See [../decisions/FEATURE_HISTORY.md#zfs_collector-phases-abc](../decisions/FEATURE_HISTORY.md#zfs_collector-phases-abc) for evidence (migrations 031 + 032 + witness consumer + coverage-gated detectors + REGIME co-occurrence wiring + live lil-nas-x DEGRADED-stable classification). Ratified 2026-05-07.
