...
- **2026-06-30 (FreeBSD lab run, live-on-lab-substrate):** built NQ on a real **FreeBSD 14.4-RELEASE-p6 amd64** VM (sushi-k libvirt/KVM, `pkg` rustc 1.94.0 — above the 1.88 MSRV). **Tier 0 (compile): clean**, all crates. **Tier 1 (capability honesty): green** — `platform_current_is_linux_on_linux_ci` asserts `Platform::current() == Other` on real FreeBSD; all three collector `non_linux_substrate_is_not_supported` tests pass; the `not_supported` persistence test passes. **`NotSupported` is a real portability primitive, not a macOS special case.** Full suite green except one test, `build_commit_returns_some_when_compiled_in_repo` — a `git archive`/tarball build artifact (no `.git` → `NQ_BUILD_COMMIT` unset; the test's own comment calls tarball `None` legitimate), reproducible on any OS built that way, **not** a portability or capability defect. This is **lab-backed compatibility + capability evidence**, not a claim of live FreeBSD support. Architectural payoff: FreeBSD and Darwin share most host-fact mechanisms (`getloadavg`, `statvfs`, `sysctl kern.boottime`/`kern.osrelease`) — see `decisions/PORTABILITY_TIER3A_HOST_ADAPTER_PARKED.md`.

- **2026-06-30 (Tier 3a landed + FreeBSD-verified, local):** macOS/FreeBSD **graduate from collector-level `not_supported` to partial-native host witnesses** — `host` now emits `status: ok` plus a typed `HostData.cannot_testify` (`HostField` enum) for fields with no honest BSD equivalent. Shared `read_bsd_facts()` (raw libc: `getloadavg`/`statvfs`/`sysctl kern.boottime`/`kern.osrelease`/`hw.memsize|hw.physmem`/`kern.bootsessionuuid`) + pure `assemble_bsd()`; Linux `/proc` path unchanged; **no `sysinfo`**; `cannot_testify` additive + wire-only (no migration). `mem_available`/`mem_pressure` always refused on BSD (no 1:1, never synthesized); `boot_id` refused on FreeBSD (no per-boot UUID), populated on Darwin. **Verified end-to-end on the FreeBSD 14.4 VM**: the `cfg(freebsd)` reader compiles and the live `collect()` smoke runs (`status: ok` + `cannot_testify` from real syscalls); full suite green (the prior `build_commit` tarball artifact eliminated by baking `NQ_BUILD_COMMIT`). `services`/`logs` remain collector-level `not_supported`. Still **not** a full Tier-3 support claim. Commit `e2b3c0b`; see `decisions/PORTABILITY_TIER3A_HOST_ADAPTER_PARKED.md`.

- **2026-06-30 (Tier 3a macOS branch verified, local):** the Darwin branch — previously written-but-uncompiled — is now **lab-verified on the M4 mac mini** (macOS 15.7.3 / Darwin 24.6.0 **arm64**, real hardware, lab box not production). `collect_on_a_supported_substrate_is_ok` passed on macOS and `Platform::current() == MacOs`; `read_bsd_facts()` executed the real Darwin MIBs — `getloadavg`, `statvfs("/")`, `kern.boottime`, `kern.osrelease`, `hw.memsize`, `kern.bootsessionuuid` — and returned `status: ok` + `cannot_testify`. `boot_id`/`mem_total_mb` populated; `mem_available_mb`/`mem_pressure_pct` absent and listed in `cannot_testify`; services/logs remain `not_supported`. No Darwin-specific fix was needed (the shared BSD reader was genuinely shared, not Darwin-shaped). **This closes the "Darwin branch written but unverified" caveat:** both FreeBSD and macOS now confirm the shared BSD partial-native host witness. Lab evidence, not production deployment testimony; still **not** a full Tier-3 support claim. Full transcript stays local at `~/nq_t3a_verify/smoke.log`; only the sanitized aggregate is recorded.
...
- `docs/working/gaps/OBSERVER_DISTORTION_GAP.md` — Δq domain; this spec extends the self-manifest concept with platform capability declarations.
