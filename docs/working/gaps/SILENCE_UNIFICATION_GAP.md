...
**Status:** `partial` — witness pair shipped 2026-06-12; see [`FEATURE_HISTORY.md#silence_unification-v1-witness-pair`](../decisions/FEATURE_HISTORY.md#silence_unification-v1-witness-pair). `smart_witness_silent` + `zfs_witness_silent` now emit the contract (derived at persist per OQ1's "finding-meta fields" option); the four non-witness detectors (`stale_host`, `stale_service`, `signal_dropout`, `log_silence`) are deferred pending OQ3/OQ4. Promoted from `ARCHITECTURE_NOTES.md §SILENCE_UNIFICATION` latent note 2026-04-27 after `smart_witness_silent` landed and brought the silence-detector count to six. Re-scoped 2026-04-28 under TESTIMONY_DEPENDENCY_GAP: the `*_witness_silent` subset of these detectors is reframed as **parent-node evidence inputs**, not peer operator alerts.
**Partial substrate landed 2026-05-12 via DURABLE_ARTIFACT_SUBSTRATE V1:** the shared envelope fields (`silence_scope`, `silence_basis`, `silence_duration_s`, `silence_expected`) shipped as optional additive `FindingSnapshot` fields, with `extraction_stale` as the first instance. SILENCE_UNIFICATION V1's primary scope (migrating the six legacy silence detectors onto these columns) remains the open work — consumers must read missing `silence` as "not yet unified", not "not silence." See [`FEATURE_HISTORY.md` § DURABLE_ARTIFACT_SUBSTRATE V1](../decisions/FEATURE_HISTORY.md#durable_artifact_substrate-v1-synthetic-producer-slice) §"SILENCE_UNIFICATION cross-gap note" for the discipline that kept this partial.
