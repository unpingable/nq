...
**Status:** `partial` — **Slice A shipped 2026-07-01** (the tombstone primitive: the one existing silent-purge path is now receipted). The **retention-windows policy is LOCKED** 2026-06-12 — see [`../decisions/NQ_RETENTION_WINDOWS.md`](../decisions/NQ_RETENTION_WINDOWS.md). **Slice handle: NQ-CLOSE-002.** Ranked #2 in the closure stack (most expensive to retrofit; decide retention windows per rung NOW while corpus is small).

**Build status (Slice boundary ratified 2026-07-01):**
- **Slice A — SHIPPED (migration 061 + `retention.rs`).** `evidence_tombstones` table (forever-retention, no FK to generations). `prune()` is now a receipted act: before the cascade delete it enumerates `generation_id`-bearing tables dynamically from `sqlite_master` (so no new history/observation table can silently escape), counts the doomed rows per table, and mints one tombstone `(generation_id_low/high, generations_deleted, rows_deleted_json, retention_rule_cited, tombstoned_at)` in the same transaction. The sweep is observable (serve.rs logs the tombstone id + range + per-table counts). Closes acceptance criteria 2 (receipted deletion of the sample/rollup cascade), 5 (observable sweep), 6 (no silent purge path). `tombstoned_at` is deliberately not `observed_at` (deletion-receipt time, not authority-freshness — cf. C2).
- **Slice B — DEFERRED (named).** Per-class time-windowed retention (raw samples 3wk vs consolidated rollups 6mo as *distinct* sweeps). Requires decoupling table lifetimes from the single generation cascade; today everything under a pruned generation dies together. Acceptance criterion 1 (per-rung windows) lands here.
- **Slice C — DEFERRED (named).** Finding basis-ladder on tombstoned evidence: transition `warning_state.basis_state` (the shipped EVIDENCE_RETIREMENT enum) when a finding's cited evidence is tombstoned, + the dangling-citation render distinction. Acceptance criteria 3, 4. Composes with [`EVIDENCE_RETIREMENT_GAP`](EVIDENCE_RETIREMENT_GAP.md), does not duplicate it.
