...
**Status:** partial — V1.0 substrate shipped 2026-04-22; **retirement verb + retired render distinction + retired notification gating shipped 2026-07-01** (the whole *explicit* half of the knife: an operator can retire a source, the retired findings render distinctly and never page). Remaining V1 follow-on: the **basis-stale detector** (the *passive* half) — gated on a `BASIS_STALE_CONTRACT` design record (OQ1) BEFORE any runtime, per operator ruling 2026-07-01 ("do not let time become a little cop"). See [../decisions/FEATURE_HISTORY.md#evidence_retirement-v1-0-substrate](../decisions/FEATURE_HISTORY.md#evidence_retirement-v10-substrate). Ratified 2026-05-07.

**Render + notification gating (retired only) — SHIPPED 2026-07-01.** `WarningVm` carries `basis_state`; `render_overview` pulls `basis_state = 'retired'` findings out of active signal (excluded from the active rows AND every header severity count) and renders them in a distinct **Retired Evidence** block with the blunt label "Retired evidence · source explicitly retired" — never OK/inactive/resolved/gone (retirement is not laundered into success or silence). `notify::find_pending` gains `AND ws.basis_state != 'retired'`, so retired findings do not page and — since the Slack payload flows from `find_pending` — never reach Slack. Restricted to `retired` on purpose: `stale`/`invalidated` are not yet set by any runtime, so gating them now would be premature. The passive basis-stale transition (`live -> stale`) that WOULD set `stale` is deferred behind its design record.

**`BASIS_STALE_CONTRACT` — RATIFIED v0 2026-07-01 (design record, NOT runtime).** See [`../decisions/BASIS_STALE_CONTRACT.md`](../decisions/BASIS_STALE_CONTRACT.md). The passive detector turns time into state ("outage cosplay as retirement"), so it is fenced by a ratified contract before any code. The 7 clauses: eligibility (only operator-ratified declared-freshness sources; no stale clock for derived/aggregator/cadence-less — "no tiny hall monitors"); declaration (static/ratified source contracts only — NQ never infers a window, a runtime payload can't extend/weaken its own); transition blockers (retired / maintenance-suppression / invalidated / unknown identity); the silence relation (same-source silence *evidences* stale but does NOT block it — dedup notifications, don't collapse the states); auto-reversibility (`stale→live` automatic, unlike `retired→unretire`); per-`basis_source_id` granularity; and the evidence threshold (`now − last_admissible_basis_observed_at > declared_window`, using the **authority** timestamp — NOT Regime B `collected_at`, per C2). **Runtime remains blocked** on a per-source-class substrate audit: a class with only `collected_at` and no authority-bearing basis observation timestamp is ineligible until that timestamp is plumbed. No detector until the audit passes.

**Retirement verb — SHIPPED 2026-07-01 (V1 slice step 3, migration 062).** `nq-monitor source retire --source-id X --reason Y` / `source unretire --source-id X`. `sources_retired` is authoritative current-state for "deliberately withdrawn"; `retire` atomically transitions every finding with matching `basis_source_id` (that is not already retired) to `retired`, writing a `finding_transitions` audit row each; idempotent (preserves the original `retired_at`/reason). `unretire` deletes the current-state row and returns findings to `unknown` — **never auto-`live`** (Invariant 7); the `finding_transitions` receipt survives, so unretire is not retroactive laundering. `publish` loads the retired-source set each cycle and keeps a re-detected finding from a retired source `retired` instead of re-living it — closing the sushi-k haunting scar (§ Problem) at the persist seam. Actor is fixed `local-operator` until identity plumbing exists (deferred, not blocking). This is the *explicit* half of the silence knife (DETECTOR_TAXONOMY §2a): retirement is explicit, silence is passive; the basis-stale detector (the passive "not heard from" half) stays a separate follow-on slice.
