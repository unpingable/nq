...
**`BASIS_STALE_CONTRACT` — design record required before the basis-stale detector is built (OQ1, operator-ruled 2026-07-01).** The passive detector turns time into state and is "extremely good at becoming outage cosplay as retirement." Before any runtime, a design record must fence: which source classes have freshness windows; who declares the window; what outage/suppression states block the `live -> stale` transition; whether `stale` is automatically reversible; whether `stale` is per-source / per-basis / per-finding; what evidence is required to transition. No detector until that contract exists.
