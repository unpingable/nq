...
- docs/working/gaps/REGIME_FEATURES_GAP.md (the semantic layer that consumes history)
- docs/working/gaps/EVIDENCE_LAYER_GAP.md (finding_observations is the sparse evidence store)
- docs/working/gaps/GENERATION_LINEAGE_GAP.md (per-generation coverage — if a chunk spans low-coverage generations, that's relevant metadata)
