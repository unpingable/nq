...
**Status:** `proposed` — drafted 2026-05-21 from clock_skew framing in `docs/working/coverage/traditional-monitoring-coverage-audit.md` (the audit's `clock_skew` detail block names this as the load-bearing half of a two-layer slice; the witness-side profile is the other half, deferred). Calibration record only. Does not authorize implementation, evaluator change, registry expansion, schema work, notification path, dashboard surface, or any code.
