...
```markdown
**Status:** shipped; see [../decisions/FEATURE_HISTORY.md#finding_diagnosis-v1](../decisions/FEATURE_HISTORY.md#finding_diagnosis-v1)
```
...
**Current ledger:** [../decisions/FEATURE_HISTORY.md#dominance_projection-v1](../decisions/FEATURE_HISTORY.md#dominance_projection-v1)
```
...
- [`WITNESS_CLAIM_SCOPE_GAP`](WITNESS_CLAIM_SCOPE_GAP.md) — `partial`. Preflight + receipt surface migrated 2026-06-09: `PreflightResult.cannot_testify` and `Receipt.cannot_testify` are now `Vec<ClaimRefusal>`; all 8 constitutional `*_cannot_testify()` functions carry typed `RefusalKind`; `PREFLIGHT_CONTRACT_VERSION` bumped 1→2. Witness-coverage surface (`SmartWitnessCoverage` / `ZfsWitnessCoverage`) explicitly **not migrated** — strings there are shape identifiers, not prose refusals; cousins-not-siblings. Standing-surface migration deferred per constraint 5 (likely the next forcing case for `ClaimRefusal`).
- [`FINDING_DIAGNOSIS_GAP`](FINDING_DIAGNOSIS_GAP.md) — `shipped` (see [FEATURE_HISTORY § FINDING_DIAGNOSIS V1](../decisions/FEATURE_HISTORY.md#finding_diagnosis-v1)). Typed finding nucleus (FailureClass, ServiceImpact, baseline action bias). Unblocks DOMINANCE_PROJECTION.
...
- [`HUMAN_NOW_FRAME_SCOPE`](HUMAN_NOW_FRAME_SCOPE.md) — `partial`. The human dashboard is a perceptual composition layer, not an evidence browser: it renders a bounded present-tense operational claim (a *Human Now Frame*) from delayed witnesses, and must never launder stale/split/unbound evidence into current green/red. Generalizes C2 two-clock host standing (ancestor); sibling to `DASHBOARD_MODE_SEPARATION_GAP` (per-panel source/render mode), which it does not absorb. **Host V0 shipped 2026-07-03** (`nq-db` `frame` module, overview card, `/api/frame/host/{host}`); Service V1 (real `split`/`settling`) and target-scoped frames (blocked on `NQ_SCRAPE_TARGET_IDENTITY_SCOPE`) pending.
- [`FLEET_INDEX_GAP`](FLEET_INDEX_GAP.md) — `code-complete` (see [FEATURE_HISTORY § FLEET_INDEX V1](../decisions/FEATURE_HISTORY.md#fleet_index-v1)). V1 cash-out of FEDERATION: comparison surface for declared NQ targets, not federation truth. One row per target with reachability, build/schema/contract metadata, target-scope support tier (`active | experimental | unsupported | observed-only`), and click-through to the local dashboard. Cross-host live smoke deferred to deploy session.

### Self-governance
- [`LOW_TOIL_SELF_OBSERVATION_GAP`](LOW_TOIL_SELF_OBSERVATION_GAP.md) — `proposed`. Parent gap for NQ's own self-observation surface (self-health endpoint, metrics exposition, structured logs, webhook custody, retention as evidentiary act, read-only API). Calibration record only; inventories which sub-gaps already cover slices. Candidate keeper: "no unbounded observer burden."
...
- [`DURABLE_ARTIFACT_SUBSTRATE_GAP`](DURABLE_ARTIFACT_SUBSTRATE_GAP.md) — `built, shipped (V1)`. **V1 shipped 2026-05-12; NS consumer-alignment dry run ratified same day** ([FEATURE_HISTORY](../decisions/FEATURE_HISTORY.md#durable_artifact_substrate-v1-synthetic-producer-slice)). Substrate-class admission for extraction-derived durable artifact corpora, distinct from live host-bound axes. Migration 046 added `origin_*` columns + SILENCE_UNIFICATION shared envelope; `nq.finding_import.v1` inbound contract with refusal-with-finding for malformed imports; `extraction_stale` detector composes onto the SILENCE_UNIFICATION envelope. Lifecycle posture: **raw passthrough with origin tag** (locked). DURABLE_ARTIFACT V1 was the forcing case that promoted the SILENCE_UNIFICATION envelope fields — six legacy silence detectors remain ad-hoc pending their own migration; consumers must read missing `silence` as "not yet unified", not "not silence". Three NS-tolerates-but-does-not-consume seams surfaced by the dry run (two-clock latency, silence-envelope invisibility, origin-envelope invisibility) named in FEATURE_HISTORY. Real-producer ingestion + PROVENANCE_GRAPH_PROFILE + corpus-shaped subject-identity are V2+/profile concerns.
...
- [`FEDERATION_GAP`](FEDERATION_GAP.md) — `candidate` / recognition. Promoted from stub 2026-06-10 with the **chain-of-custody-at-fan-in** doctrine: samples stay home, claims travel; subpoena-on-adjudication; tree-for-custody + rings-for-liveness; *a parent may compose child testimony, but may not convert it into parent observation*; read-only-upward as constitution. Pins NQ-FED-000 as the cheap-now provenance-widening slice (no federation built; finding provenance widened so future federation isn't gated on un-provenance'd inheritance). **Sequenced LAST in closure stack** (operator ranking 2026-06-10); see [`../decisions/NQ_CLOSURE_STACK.md`](../decisions/NQ_CLOSURE_STACK.md).
- [`OPERATOR_ATTESTATION_GAP`](OPERATOR_ATTESTATION_GAP.md) — `candidate` / recognition. **Slice NQ-CLOSE-001** (operator-ranked #1 in closure stack 2026-06-10). *The operator is a witness.* Pins `nq attest` CLI shape, `operator_attestation.v1` JSON record candidate, and the load-bearing `effect_claimed: false | true` distinction ("I touched X" vs "this fixed X" are different claims). Closes the human-shaped ghost channel — manual SSH fixes, PRAGMA wal_checkpoints, vi config edits — that every estate accretes invisibly. Founding keeper: *NQ witnesses machine state, but machine state is not changed only by machines.*
- [`EVIDENCE_FORGETTING_GAP`](EVIDENCE_FORGETTING_GAP.md) — `candidate` / recognition. **Slice NQ-CLOSE-002** (operator-ranked #2 — most expensive to retrofit). *Evidence may expire; citations may not silently dangle. Expiration changes admissibility, not history.* Pins the per-rung retention table (samples weeks / rollups months / findings forever / attestations alongside findings / tombstones forever) and the three-state finding ladder (active_with_evidence → active_evidence_expired → retired_evidence_expired). Tombstones as receipted deletion. Composes with SURFACE_TYPED_REVOCATION_CANDIDATE's Lean-proved unlinked-not-closed kernel; complements shipped EVIDENCE_RETIREMENT_GAP (which governs finding-state rendering when basis is gone) by adding the policy-side that decides *when* basis goes.
- [`HOST_TRUST_BOUNDARY`](HOST_TRUST_BOUNDARY.md) — `doc-only` constitutional note. **Slice NQ-CLOSE-003** (operator-ranked #3 — doc-only for now). One paragraph: *NQ's local witness trusts the host on which it runs; tamper-evidence begins after collection; it does not defeat root, kernel, or malicious-local-operator compromise; cross-host and hostile-host assurance are separate higher-rung problems.* Refuses hash-chain maximalism (*"anything more becomes crypto cosplay"*). Names what NQ structurally does not pretend to overcome.
