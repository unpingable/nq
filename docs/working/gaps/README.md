...
- [`WITNESS_CLAIM_SCOPE_GAP`](WITNESS_CLAIM_SCOPE_GAP.md) — `specified, ready to implement`. Refusals (`cannot_testify`) are currently `Vec<String>` across three parallel surfaces (witness observation coverage, evaluator PreflightResult, witness standing). Slice promotes them to typed `ClaimRefusal { refusal_kind, statement }` so consumers can bind to refusal identity without parsing prose. Driver is completeness on an already-emitting surface — not a new consumer trigger. Standing-surface migration deferred per implementation constraint 5. Substrate-coverage piece composes with `SUBSTRATE_COVERAGE_DECLARATION_GAP`.
