...
- [`WITNESS_CLAIM_SCOPE_GAP`](WITNESS_CLAIM_SCOPE_GAP.md) — `proposed`. Refusals (`cannot_testify`) are currently `Vec<String>` across three parallel surfaces (witness observation coverage, evaluator PreflightResult, witness standing). Calibration record for promoting them to typed `ClaimRefusal { refusal_kind, statement }` so consumers can bind to refusal identity without parsing prose. Vocabulary correction (observation / claim / finding / receipt / refusal) is recognition, not new doctrine. Substrate-coverage piece composes with existing `SUBSTRATE_COVERAGE_DECLARATION_GAP`.
