...
**Status:** shipped; see [../decisions/FEATURE_HISTORY.md#finding_export-v1](../decisions/FEATURE_HISTORY.md#finding_export-v1) for shipped commits, evidence, and what's unblocked. The Shipped State narrative below remains as design-record content (what was deferred, what was discovered).
...
- `docs/working/gaps/EVIDENCE_LAYER_GAP.md` — per-generation observation substrate that this export surface reads from.
- `docs/working/gaps/FINDING_DIAGNOSIS_GAP.md` — typed diagnosis fields exposed in the DTO.
- `docs/working/gaps/REGIME_FEATURES_GAP.md` — trajectory / persistence / recovery payload semantics.
- `docs/working/gaps/OBSERVER_DISTORTION_GAP.md` — sibling discipline for probes; the consumer-side rule here extends the same invariant to readers.
- `docs/working/gaps/DASHBOARD_MODE_SEPARATION_GAP.md` — snapshot-is-evidence framing originated there; this gap applies it to a programmatic surface.
