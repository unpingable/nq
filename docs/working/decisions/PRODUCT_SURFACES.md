...
NQ's internal doctrine (admissibility, cannot_testify, witness standing, claim preflight, the eight verdicts) is load-bearing in the code and in `docs/working/decisions/CLAIM_PREFLIGHT.md` / `docs/operator/VERDICTS.md` / `docs/architecture/WITNESS_PACKET.md`. It does not appear in user-facing output.
...
This document orients implementation. Phase-specific implementation still lands through focused specs or gap records where retrofit cost warrants it (e.g. `docs/working/gaps/DISK_STATE_CUTOVER_TO_SHARED_SPINE.md`, `docs/working/gaps/TRACK_B_WITNESS_PRODUCERS.md` — neither yet written). The existing `docs/working/gaps/AGENTIC_CI_WITNESS_FAMILIES_GAP.md` already pins Track B's constitutional witness-shape bounds and remains authoritative for that surface.
...
- `docs/working/decisions/CLAIM_PREFLIGHT.md` — internal doctrine: the ladder and the finding ≠ claim cut.
- `docs/operator/VERDICTS.md` — internal eight-verdict vocabulary.
- `docs/architecture/WITNESS_PACKET.md` — witness-semantics constraints (proxy shock / replicated observability / timestamped evidence).
- `docs/working/decisions/MVP_SCOPE.md` — v0 don't-build list.
- `docs/working/gaps/AGENTIC_CI_WITNESS_FAMILIES_GAP.md` — Track B's constitutional witness bounds.
- `docs/working/gaps/CLAIM_KIND_DISK_STATE_GAP.md` — Track A disk-state substrate/workflow split.
