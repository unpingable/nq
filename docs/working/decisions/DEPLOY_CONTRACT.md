...
### B. Public reverse proxy supervision — RESOLVED 2026-06-28 (Packet 4b)
- **Corrected characterization:** the public proxy is a **shared, jbeck-owned docker `caddy:2` container**, not an nq-specific one. Its real config is `/home/jbeck/atproto/Caddyfile` (NOT `/root/Caddyfile`, which is a stale 186-byte decoy). It reverse-proxies ~8 domains across several projects — `juche.social` (atproto PDS), `sp00ky.net`, `labelwatch`, `driftwatch`, `instantinternet.news`, `stechometer`, and **`nq.neutral.zone` → `172.17.0.1:9848`** (with a defensive 405 holding move on unauthenticated `/api/finding/transition` + `/api/saved` writes). `via: 1.1 Caddy` + a Let's Encrypt cert (CN=nq.neutral.zone) confirm caddy terminates nq's TLS.
- **It was already reboot-survivable** (container `restart=unless-stopped` + `docker.service` enabled) — the original "won't survive reboot" worry was unfounded. The real gap was *named/explicit/declared* management.
- **Fix:** added `/etc/systemd/system/caddy-proxy.service` (reference copy: `deploy/examples/caddy-proxy.service`) that manages the **existing** container in place (`docker start -a caddy` — never recreates, so the `/data` LE certs for all 8 domains are preserved). Container restart policy set to `no` so systemd is the single supervisor. Unit `enabled` (boot) + `active`; `systemctl restart` proven clean (container cycles, all tenants recover); public nq 200, leak scan clean; no tenant dropped (juche/labelwatch/instantinternet/stechometer 200, driftwatch 404 unchanged). Rollback documented in the unit header. Receipt: `.governor/loop-receipts/2026-06-28T2025Z.caddy-supervision.json`.
- **Scope held:** transport plumbing only — did not touch NQ app semantics, TLS 2d-b, run-as-user/UMask, or backup retention. *This packet supervises the pipe; it does not certify the observations served through it.*
