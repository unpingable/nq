...
- **Backends:** memfile reader (`parse_kea_leases`) **and** control-socket reader (`kea_control`, `lease4-get-all`) both landed — same `KeaLease` shape, cross-backend-consistency tested. Live SSH wiring of the socket backend (`nc -U` over SSH) + real-Kea test are gated. Deferred: `stat-lease4-get` pool utilization; canonical witness-JSON emission; the `dhcp_dns_identity_consistency` composite (named, not built — never DHCP laundering DNS into existence).
